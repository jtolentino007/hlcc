<?php
	...
	class Visitors extends CORE_Controller
	{
		...
		public function index()
		{
        	...
        	$data['title'] = 'Visitors - His Life City Church';

        	$data['schools'] = $this->School_model->get_list(array('schools.is_active'=>TRUE,'schools.is_deleted'=>FALSE));
		...
		function transaction($txn = null) {
	        switch ($txn) {
	            ...
	            case 'create':
                	...
                	$m_visitor->school_id = $this->input->post('school_id', TRUE);
	                ...
	                $response['row_added'] = $this->response_rows($visitor_id,
	                	'visitors.*, CONCAT(visitor_fname, " ", visitor_mname, " ", visitor_lname) AS fullname'
	                );
	                echo json_encode($response);
	            	break;
	            ...
	            case 'update':
	                ...
	                $response['row_updated'] = $this->response_rows($visitor_id,
	                	'visitors.*, CONCAT(visitor_fname, " ", visitor_mname, " ", visitor_lname) AS fullname'
	                );
	                echo json_encode($response);
	                break;
	    ...
	    function response_rows($filter){
	    	return $this->Visitors_model->get_list(
	    		$filter,

	    		'visitors.*,CONCAT(visitor_fname, " ", visitor_mname, " ", visitor_lname) AS fullname,schools.school_name',

	    		array(
	    			array('schools','schools.school_id=visitors.school_id','left')
	    		)
	    	);
	    }
	}
